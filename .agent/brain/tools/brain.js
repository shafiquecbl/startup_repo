 ...
 *   node brain.js scan "Theme.of" --name theme    # scan + generate checklist
 *   node brain.js prune                           # archive stale nodes
...
// ============================================================
// SCAN — Find pattern occurrences and generate checklist
// ============================================================
function scan(pattern, name, ext = '.dart') {
  const libDir = path.join(PROJECT_ROOT, 'lib');
  if (!fs.existsSync(libDir)) { console.error(`No lib/ directory at ${PROJECT_ROOT}.`); process.exit(1); }

  const regex = new RegExp(pattern, 'gi');
  const files = findFiles(libDir, ext);
  const hits = [];
  ...
  for (const file of files) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (regex.test(lines[i])) {
        fileHits.push({ line: i + 1, text: lines[i].trim().slice(0, 120) });
      }
      regex.lastIndex = 0;
    }
    if (fileHits.length > 0) {
      hits.push({ file: relPath, matches: fileHits });
    }
  }

  if (hits.length === 0) {
    console.log(`No matches for pattern "${pattern}" in ${ext} files.`);
    return;
  }

  const totalMatches = hits.reduce((sum, h) => sum + h.matches.length, 0);
  const slug = (name || pattern).replace(/[^a-zA-Z0-9]/g, '-').toLowerCase().slice(0, 50);

  // Generate checklist markdown
  const checklistDir = path.join(AGENT_DIR, 'plan', 'checklists');
  if (!fs.existsSync(checklistDir)) fs.mkdirSync(checklistDir, { recursive: true });
  const checklistPath = path.join(checklistDir, `${slug}.md`);

  let md = `# Checklist: ${name || pattern}\n\n`;
  md += `> Auto-generated by brain scan — ${new Date().toISOString().slice(0, 10)}\n`;
  md += `> Pattern: \`${pattern}\`\n`;
  md += `> Total: ${totalMatches} matches in ${hits.length} files\n`;
  md += `> Progress: 0/${hits.length} files done\n\n`;
  ...
  md += `---\n\n`;
  md += `## Execution notes\n`;
  md += `_AI: Update [ ] to [x] after fixing each file. Write notes here if needed._\n`;

  fs.writeFileSync(checklistPath, md);
  console.log(`Scan complete: ${totalMatches} matches in ${hits.length} files\n`);
  console.log(`Checklist saved: ${path.relative(PROJECT_ROOT, checklistPath)}\n`);

  // Print summary
  for (const h of hits) {
    console.log(`  ${h.file} (${h.matches.length} match${h.matches.length > 1 ? 'es' : ''})`);
  }
}

// ============================================================
// PRUNE — Remove nodes whose file_path no longer exists
// ============================================================
async function prune() {
  const db = await getDb();
  const nodes = queryAll(db, `SELECT id, name, file_path FROM nodes WHERE file_path IS NOT NULL AND status = 'active'`);
  let removed = 0;

  for (const n of nodes) {
    const fullPath = path.join(PROJECT_ROOT, n.file_path);
    if (!fs.existsSync(fullPath)) {
      exec(db, `UPDATE nodes SET status = 'archived', updated_at = datetime('now') WHERE id = ?`, [n.id]);
      console.log(`  Archived: [${n.id}] ${n.name} — ${n.file_path} (file deleted)`);
      removed++;
    }
  }
  ...
  if (removed > 0) {
    ...
    console.log('No stale nodes found. All file paths are valid.');
  }
}
...
async function main() {
  ...
  switch (cmd) {
    ...
    case 'scan': {
      ...
      const ext = extIdx !== -1 ? args[extIdx + 1] : '.dart';
      const pattern = args.filter((a, i) => a !== '--name' && a !== '--ext' && (nameIdx === -1 || i !== nameIdx + 1) && (extIdx === -1 || i !== extIdx + 1)).slice(1).join(' ');
      if (!pattern) { console.error('Usage: brain.js scan "pattern" [--name checklist-name] [--ext .dart]'); process.exit(1); }
      scan(pattern, name, ext); break;
    }
